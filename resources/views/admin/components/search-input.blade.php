...
<div x-cloak class="{{ $containerClass }}">
    <input id="{{ $inputId }}" name="{{ $name }}" type="text" placeholder="{{ $placeholder }}"
        value="{{ old($name, $value) }}" autocomplete="off"
        class="w-full rounded-lg border px-3 py-2 text-gray-900 placeholder:text-gray-400 focus:outline-none px-2 focus:ring-4 transition {{ $class }}"
